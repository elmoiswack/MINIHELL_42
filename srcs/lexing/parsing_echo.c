...
t_lexer	*echo_meta_intolist(t_lexer *info_list, char **splitted_line, int *enum_array, int index)
{
		...
		is_metacharachter(splitted_line[index][0]) != 1)
	{
		...
		if (!splitted_line[index])
			return (error_lex(info_list, 3, "parsing_echo.c/L20"), NULL);
		info_list->content[1] = ft_strjoin_and_free(info_list->content[1], \
			splitted_line[index]);
		if (!info_list->content[1])
			return (error_lex(info_list, 3, "parsing_echo.c/L23"), NULL);
		if (splitted_line[index + 1] != NULL)
			info_list->content[1] = ft_strjoin_and_free(info_list->content[1], \
				" ");
		if (!info_list->content[1])
			return (error_lex(info_list, 3, "parsing_echo.c/L28"), NULL);
		index++;
	}
